...
class Stroke
{
     ...
     * 获取汉字的笔画数
     *
     * @param string $str 汉字，可以不止一个汉字
     * @return array 按照汉字顺序输出笔画数组
     * @throws \InvalidArgumentException
     */
    public static function get($str)
    {
        $str = trim($str);
        $strArr = Str::toArray($str);
        $results = [];
        foreach($strArr as $s) {
            $results[] = self::getSingle($s);
        }
     ...
     * @param string $str 汉字
     * @return int 笔画数
     * @throws \InvalidArgumentException 如果参数不是一个汉字
     */
    public static function getSingle($str)
    {
        $str = trim($str);
        if(Str::length($str) > 1) {
            throw new \InvalidArgumentException('getSingle method only support one chinese word.');
        }
        //不是中文，笔画为0
        if(!Str::isChinese($str)) {
            return 0;
        }
        foreach(self::$lib as $k=>$v) {
